...
contract ExercisePreHectagon is Pausable, Ownable{
  ...
  uint256 immutable public blockStart;
  /* ====== SPACE CALCULATION ====== */
  /**
  ...
  * tokenCurrentSpaceInfo[tokenAddress] = CurrentSpace
  ...
  IHECTA immutable public hecta;
  IERC20 immutable public busd;
  ITreasury public immutable treasury;
  PHectagonERC20 immutable public pHecta;
  THectagonERC20 immutable public tHecta;
  ...
  function exercise( address tokenAddress, uint _amount ) external whenNotPaused returns ( bool ) {
    require( tokenAddress == address(pHecta) || tokenAddress == address(tHecta), 
      "ExercisePreHectagon: Wrong contract address" );
    require( PreHectagonERC20(tokenAddress).balanceOf(msg.sender) >= _amount, 
    ...
    if(currentSpaceIndex > info.lastSpaceIndex){
      info.totalMintableInCurrentSpace = 
        // percentage of token owned by msg.sender
        PreHectagonERC20(tokenAddress).balanceOf(msg.sender)
        ...
        mul(tokenCurrentSpaceInfo[tokenAddress].markedHectaTotalSupply).div(spaceCapMint);
      // TODO: this math is prone to error & hack
      info.lastSpaceIndex = currentSpaceIndex;
    }

    require(_amount < info.totalMintableInCurrentSpace, 
      "ExercisePreHectagon: Exceeds quota in this space");
