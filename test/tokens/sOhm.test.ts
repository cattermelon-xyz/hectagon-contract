...
describe("sHecta", () => {
  ...
  beforeEach(async () => {
    ...
    const authority = await (new HectagonAuthority__factory(initializer)).deploy(initializer.address, initializer.address, initializer.address, initializer.address);
  ...
  describe("initialization", () => {
    ...
    describe("initialize", () => {
      ...
      it("emits Transfer event", async () => {
        await expect(sHecta.connect(initializer).initialize(stakingFake.address, treasuryFake.address)).
          to.emit(sHecta, "Transfer").withArgs(ZERO_ADDRESS, stakingFake.address, TOTAL_GONS);
      });

      it("emits LogStakingContractUpdated event", async () => {
        await expect(sHecta.connect(initializer).initialize(stakingFake.address, treasuryFake.address)).
          to.emit(sHecta, "LogStakingContractUpdated").withArgs(stakingFake.address);
      });

      it("unsets the initializer, so it cannot be called again", async () => {
        await sHecta.connect(initializer).initialize(stakingFake.address, treasuryFake.address);
        await expect(sHecta.connect(initializer).initialize(stakingFake.address, treasuryFake.address)).to.be.reverted;
      });
    });
  });
    ...
    describe("approve", () => {
      ...
      it("emits an Approval event", async () => {
        await expect(await sHecta.connect(alice).approve(bob.address, 10)).
          to.emit(sHecta, "Approval").withArgs(alice.address, bob.address, 10);
      });
    });
    ...
    describe("increaseAllowance", () => {
      ...
      it("emits an Approval event", async () => {
        ...
        await expect(await sHecta.connect(alice).increaseAllowance(bob.address, 4)).
          to.emit(sHecta, "Approval").withArgs(alice.address, bob.address, 14);
      });
    });
    ...
    describe("decreaseAllowance", () => {
      ...
      it("emits an Approval event", async () => {
        ...
        await expect(await sHecta.connect(alice).decreaseAllowance(bob.address, 4)).
          to.emit(sHecta, "Approval").withArgs(alice.address, bob.address, 6);
      });
    });
