...
describe("Private Hectagon", async () => {
    ...
    beforeEach(async () => {
        ...
        pHecta = await new PHectagon__factory(owner).deploy(hecta.address, treasury.address);

        await pHecta.setBusdAddress(busd.address);
    ...
    describe("stopMigration", () => {
        it("must be done by owner", async () => {
            await expect(pHecta.connect(bob).stopMigration()).to.be.revertedWith(
        ...
        it("Owner can be done correctly", async () => {
            ...
            await ownerConn.stopMigration();
            await expect(await ownerConn.isMigrationDone()).to.be.equal(true);
        });
    });

    describe("mint", () => {
        it("must be done by owner", async () => {
            await expect(pHecta.connect(bob).mint(bob.address, 100)).to.be.revertedWith(
                "Ownable: caller is not the owner"
            );
        });

        it("can be done correctly by owner before disable migration", async () => {
            const amount = 100;
            await pHecta.connect(owner).mint(alice.address, amount);
            const aliceBalacne = await pHecta.connect(owner).balanceOf(alice.address);
            await expect(aliceBalacne).to.be.equal(amount);
        });

        it("Owner can NOT done after migration disabled", async () => {
            const amount = 100;
            const ownerConn = pHecta.connect(owner);
            await ownerConn.stopMigration();
            await expect(ownerConn.mint(bob.address, amount)).to.be.revertedWith(
    ...
    describe("setBusdAddress", () => {
        it("must be done by owner", async () => {
            await expect(pHecta.connect(bob).setBusdAddress(busd.address)).to.be.revertedWith(
                "Ownable: caller is not the owner"
            );
        });
        ...
        it("Owner can be done correctly", async () => {
            const ownerConn = pHecta.connect(owner);
            await ownerConn.setBusdAddress(busd.address);
            await expect(await ownerConn.busdAddress()).to.be.equal(busd.address);
        });
    });
    ...
    describe("start", () => {
        ...
        it("Owner can be done correctly", async () => {
            ...
            const spaceCount = await ownerConn.spaceCount();
            await expect(spaceCount).to.be.eq(0);

            const [totalHecta, totalPHecta, timestamp] = await ownerConn.spaces(spaceCount);
            await expect(totalHecta).to.be.eq(0);
            await expect(totalPHecta).to.be.eq(0);
            await expect(timestamp).to.be.eq(timestampBefore);
        });
    });

    describe("showClaimable()", () => {
        const mockBalance = BigNumber.from("1000000000000"); // 1000 pHecta

        beforeEach(async () => {
            const ownerConn = pHecta.connect(owner);
            await pHecta.mint(alice.address, mockBalance);
            await pHecta.mint(bob.address, mockBalance);
            await ownerConn.start();
        });
        ...
        it("show info correctly before start", async () => {
            ...
            const aliceClaimable = await ownerConn.showClaimable(alice.address);
            const bobClaimable = await ownerConn.showClaimable(bob.address);
    ...
    describe("beat()", () => {
        it("after first space, space meta data should updated", async () => {
            ...
            const currentSpaceCount = await bobConn.spaceCount();
    ...
    describe("transfer", () => {
        const aliceMockBalance = BigNumber.from("3000000000000"); // 3000 pHecta
        const bobMockBalance = BigNumber.from("1000000000000"); // 1000 pHecta

        beforeEach(async () => {
            const ownerConn = pHecta.connect(owner);
            await pHecta.mint(alice.address, aliceMockBalance);
            await pHecta.mint(bob.address, bobMockBalance);
            await ownerConn.start();
        });
        ...
        it("user can transfer correctly after start in space 1", async () => {
            const transferAmount = BigNumber.from("3000000000000");
            ...
            const aliceInfo = await ownerConn.infos(alice.address);
            const bobInfo = await ownerConn.infos(bob.address);

            const aliceCurrentSpaceProfit = premintHecta
                .mul(MaxPHectaToExercise)
                .div(RateDenominator)
                .mul(aliceMockBalance)
                .div(aliceMockBalance.add(bobMockBalance));
        ...
        it("user can transfer correctly after start in space 1, totalHecta incre then space 2 transfer called, then address info updated correctly", async () => {
            const ownerConn = pHecta.connect(owner);
            const transferAmount = BigNumber.from("1000000000000"); // 1000 pHecta
    ...
    describe("exercise", () => {
        const aliceMockBalance = BigNumber.from("4000000000000"); // 4000 pHecta
        const bobMockBalance = BigNumber.from("1000000000000"); // 1000 pHecta
        ...
        it("after exercise, next space user can transfer correctly", async () => {
            ...
            const space2AliceCurrentSpaceProfit = BigNumber.from(additionHectaAmount)
                ...
                .div(RateDenominator)
                .mul(space2AliceBalance)
                .div(aliceMockBalance.add(bobMockBalance).sub(exercisedPHecta));
            const maxClaimTransfer = space2AliceCurrentSpaceProfit
                .mul(transferAmount)
                .div(space2AliceBalance);
