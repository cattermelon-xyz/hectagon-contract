...
describe("PHecta and THecta is PreHectagonERC20 so both of them", function() {
  ...
  it("should let Owner mint new token if allowMinting=true", async function(){
    ...
    expect(await phecta.connect(deployer).mint(bob.address, noOfNewToken))
      .to.emit(phecta, "TokenMinted").withArgs(bob.address, noOfNewToken);
    ...
    expect(await thecta.connect(deployer).mint(bob.address, noOfNewToken))
      .to.emit(thecta, "TokenMinted").withArgs(bob.address, noOfNewToken);
  ...
  it("shouldn't let Owner mint new token if allowMinting=false", async function(){
     ...
     * testing on phecta
     */
    await expect(phecta.connect(deployer).disableMinting()).
      to.emit(phecta, "MintingDisabled");
    await expect(phecta.connect(deployer).disableMinting()).
      to.be.revertedWith("Minting has been disabled.");
    await expect(phecta.connect(deployer).mint(bob.address, noOfNewToken))
      .to.be.revertedWith("Minting has been disabled.");
    /**
     * testing on thecta
     */
    await expect(thecta.connect(deployer).disableMinting()).
      to.emit(thecta, "MintingDisabled");
    await expect(thecta.connect(deployer).disableMinting()).
      to.be.revertedWith("Minting has been disabled.");
    await expect(thecta.connect(deployer).mint(bob.address, noOfNewToken))
      .to.be.revertedWith("Minting has been disabled."); 
  })
  ...
  it("should let Owner pause all token transfer and no one (incl. owner) can tranfer anything", async function(){
    ...
    let noOfTokenToTransfer = _getNoOfToken(0.03);
    /**
     * testing on phecta
     */
    // transfer something to alice
    await expect(phecta.connect(deployer).transfer(alice.address, noOfTokenToTransfer)).
      to.emit(phecta, "Transfer").withArgs(deployer.address, alice.address, noOfTokenToTransfer);
    // pause
    await expect(phecta.connect(deployer).pause()).
      to.emit(phecta, "Paused").withArgs(deployer.address);
    ...
    await expect(thecta.connect(deployer).transfer(alice.address, noOfTokenToTransfer)).
      to.emit(thecta, "Transfer").withArgs(deployer.address, alice.address, noOfTokenToTransfer);
    // pause
    await expect(thecta.connect(deployer).pause()).
      to.emit(thecta, "Paused").withArgs(deployer.address);
    // deployer try to transfer and return error
    await expect(thecta.connect(deployer).transfer(alice.address, noOfTokenToTransfer)).
      to.revertedWith("Pausable: paused");
  ...
  it("should let Owner unpause token transfer and everyone can transfer/burn token normally", async function(){
    ...
    await expect(phecta.connect(deployer).unpause()).
      to.emit(phecta, "Unpaused").withArgs(deployer.address);
    await expect(phecta.connect(deployer).transfer(alice.address, noOfTokenToTransfer)).
      to.emit(phecta, "Transfer").withArgs(deployer.address, alice.address, noOfTokenToTransfer);
    await expect(phecta.connect(alice).transfer(bob.address, noOfTokenToTransfer)).
      to.emit(phecta, "Transfer").withArgs(alice.address, bob.address, noOfTokenToTransfer);
    ...
    await expect(thecta.connect(deployer).unpause()).
      to.emit(thecta, "Unpaused").withArgs(deployer.address);
    await expect(thecta.connect(deployer).transfer(alice.address, noOfTokenToTransfer)).
      to.emit(thecta, "Transfer").withArgs(deployer.address, alice.address, noOfTokenToTransfer);
    await expect(thecta.connect(alice).transfer(bob.address, noOfTokenToTransfer)).
      to.emit(thecta, "Transfer").withArgs(alice.address, bob.address, noOfTokenToTransfer);
  })
  ...
  it("should let Owner pause but token owner still can approve others spend his token after Owner unpause", async function(){
    ...
    expect(await phecta.connect(alice).approve(bob.address, _getNoOfToken(1000))).
      to.emit(phecta, "Approval").withArgs(alice.address, bob.address, _getNoOfToken(1000));
    // unpause
    expect(await phecta.connect(deployer).unpause()).
      to.emit(phecta, "Unpaused").withArgs(deployer.address);
    expect(await phecta.connect(bob).transferFrom(alice.address,harman.address, noOfTokenToTransfer/2)).
      to.emit(phecta, "Transfer").withArgs(alice.address, harman.address, noOfTokenToTransfer/2);
    ...
    expect(await thecta.connect(alice).approve(bob.address, _getNoOfToken(1000))).
      to.emit(thecta, "Approval").withArgs(alice.address, bob.address, _getNoOfToken(1000));
    // unpause
    expect(await thecta.connect(deployer).unpause()).
      to.emit(thecta, "Unpaused").withArgs(deployer.address);
    expect(await thecta.connect(bob).transferFrom(alice.address,harman.address, noOfTokenToTransfer/2)).
      to.emit(thecta, "Transfer").withArgs(alice.address, harman.address, noOfTokenToTransfer/2);
  })
});